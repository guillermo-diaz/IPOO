<?php
class Viaje{
  private $codigo, $destino, $max_pasajeros, $pasajeros, $responsable, $costo, $costo_total_abonado;

  public function  __construct($codigo, $destino, $max_pasajeros, $pasajeros, $respon, $costo, $total_abonado){
    if (!empty($codigo) && !empty($destino) && ($max_pasajeros >= count($pasajeros))){
      ...
      $this->costo = $costo;
      $this->costo_total_abonado = $total_abonado;
    } else{
  ...
  public function hayPasajesDisponible(){
    return count($this->getPasajeros()) < $this->getMax_pasajeros();
  }
  
  //agrega un pasajero al viaje actual
  public function venderPasaje($pas){
    $costo = -1;

    if ($this->hayPasajesDisponible() && !$this->esta_en_viaje($pas)){ //si no esta lleno y no esta ya en el viaje
      $this->pasajeros[count($this->pasajeros) - 1] = $pas;
      $costo = $this->getCosto() * ((100 + $pas->darPorcentajeIncremento())/100);
      $this->setCosto_total_abonado($this->getCosto_total_abonado() +$costo);
  ...
  /**
   * Get the value of costo_total_abonado
   */ 
  public function getCosto_total_abonado()
  {
    return $this->costo_total_abonado;
  }

  /**
   * Set the value of costo_total_abonado
   *
   * @return  self
   */ 
  public function setCosto_total_abonado($costo_total_abonado)
  {
    $this->costo_total_abonado = $costo_total_abonado;
